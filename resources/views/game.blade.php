    ...
    <body>
        ...
        @if ($game->current_stage == 'intro')
            <p>You are the world-famous spy Axel Huntington. Your mission is to infiltrate the Russian base in Siberia and
                execute a scientist named Pavlov who is close to perfecting his mind control serum. The base seems to be
                very
        ...
        @elseif ($game->current_stage == 'climb_wall')
            ...
            <form action="{{ route('game.progress') }}" method="POST">
                ...
                <button name="choice" value="sneak_in">Take out the guard sitting by the campfire and take his
                    uniform</button>
            </form>
        @elseif ($game->current_stage == 'avoid_guards')
            ...
            <form action="{{ route('game.progress') }}" method="POST">
                ...
                <button name="choice" value="explore_area">Explore the area in the hope of finding something
                    useful</button>
            </form>
        @elseif ($game->current_stage == 'take_out')
            <p>You take out the guard and take his uniform. You manage to sneak past the guards and find yourself in the
                main
        ...
        @elseif ($game->current_stage == 'surrender_guards')
            <p>You surrender to the guards. They take you inside the base and lock you up in a cell. You are unable to
                escape
        ...
        @elseif ($game->current_stage == 'hide_convoy')
            <p>You hide under the convoy and are successfully transported inside the base. You now arrive inside the
                warehouse,
        ...
        @elseif ($game->current_stage == 'find_a_exit')
            <p>There’s an open door in the warehouse. After going through it, you enter a hallway. There seems to be a guard
                at
                the end
        ...
        @elseif ($game->current_stage == 'attack_guard')
            <p>You swiftly take out the guard and go through the door. Once behind that door, there's a large open area with
                a
                laboratory. There are several scientists in there, but they spot you along with the guards. What do you do?
            </p>
            <form action="{{ route('game.progress') }}" method="POST">
                @csrf
            ...
            <form action="{{ route('game.progress') }}" method="POST">
                ...
                <button name="choice" value="shoot">Grab your weapon and shoot at everyone in the lab, hoping to kill
                    Pavlov
                    in time</button>
            </form>
        ...
        @elseif ($game->current_stage == 'surrender')
            <p>You surrender to the guards. They take you to a cell where you are locked up. You are unable to escape and
                the
        ...
        @elseif ($game->current_stage == 'destroy_serum')
            <div class="bg-light p-4 rounded shadow-sm">
                <p class="mb-4">You manage to destroy the serum and capture Pavlov. You then make your way out of the
                    base.
        ...
        @elseif ($game->current_stage == 'explore_area')
            <div class="bg-light p-4 rounded shadow-sm">
                ...
                <form action="{{ route('game.progress') }}" method="POST">
                    ...
                    <button type="submit" class="btn btn-info" name="choice" value="skip_disguise">Continue without
                        the
                        disguise</button>
                </form>
            </div>
        @elseif ($game->current_stage == 'wear_disguise')
            <div class="bg-light p-4 rounded shadow-sm">
                <p class="mb-4">You wear the guard’s uniform and manage to sneak past the guards. You find yourself in
                    the
                ...
                <form action="{{ route('game.progress') }}" method="POST">
                    @csrf
                    <input type="hidden" name="next_stage" value="head_to_target">
                    <button type="submit" class="btn btn-info" name="choice" value="head_to_target">Head straight to
                        your
                        target</button>
                </form>
            </div>
        ...
        @elseif ($game->current_stage == 'choose_item')
            <div class="bg-light p-4 rounded shadow-sm">
                ...
                <form action="{{ route('game.progress') }}" method="POST">
                    @csrf
                    <input type="hidden" name="next_stage" value="final_mission">
                    <button type="submit" class="btn btn-secondary" name="item" value="weapon">Take the
                        weapon</button>
                </form>
            </div>
        ...
        @elseif ($game->current_stage == 'throw_a_grenade')
            <div class="bg-light p-4 rounded shadow-sm">
                ...
                <form action="{{ route('game.progress') }}" method="POST" class="mb-2">
                    ...
                    <button type="submit" class="btn btn-danger" name="choice" value="hide_behind_door">Hide behind
                        the
                        lab door</button>
                </form>
                <form action="{{ route('game.progress') }}" method="POST">
                    ...
                    <button type="submit" class="btn btn-danger" name="choice" value="run_away">Run away in the hope
                        of
                        escaping</button>
                </form>
            </div>
        @elseif ($game->current_stage == 'hide_behind_door')
            <div class="bg-light p-4 rounded shadow-sm">
                <p class="mb-4">You hide behind the lab door, but the explosion is too powerful. The chain reaction from
                    the
                    equipment's explosion causes the entire lab to collapse, and you are caught in the blast. Unfortunately,
                    you
        ...
        @elseif ($game->current_stage == 'run_away')
            <div class="bg-light p-4 rounded shadow-sm">
                <p class="mb-4">You sprint away from the explosion. As you run through the corridors, you hear the
                    massive
                    explosion behind you. The entire base is engulfed in flames and debris. You manage to escape the
                    destruction
